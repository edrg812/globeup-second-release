...
const CreateOrEditPixele = ({previousData}) => {
  const [formData, setFormData] = useState({
    pixelsId: previousData?.pixelsId || '',
  ...
  const validateForm = () => {
    ...
    if (!formData.pixelsId.trim()) {
      newErrors.pixelsId = 'Pixels ID is required';
    }
    ...
    <div className="bg-violet-50 min-h-screen font-sans">
      <div className="min-h-screen">
        <header className="p-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-800">{previousData ? 'Edit Pixel' : 'Create Pixel'}</h1>
          <Link
            to="/admin/settings/pixels/manage"
        ...
        <main className="p-4">
          <div className="max-w-lg mx-auto bg-white rounded-xl shadow-lg p-6 md:p-8 mt-6 md:mt-10">
            <form onSubmit={handleSubmit}>
              {/* Pixels ID Field */}
              <div className="mb-6">
                <label className="block text-gray-700 text-sm font-medium mb-2" htmlFor="pixels-id">
                  Pixels ID *
                </label>
                <input
                  id="pixels-id"
                  name="pixelsId"
                  type="text"
                  value={formData.pixelsId}
                  onChange={handleInputChange}
                  className={`w-full px-4 py-3 border ${errors.pixelsId ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-400`}
                  placeholder="Enter your Pixels ID"
                />
                {errors.pixelsId && (
                  <p className="mt-1 text-sm text-red-600">{errors.pixelsId}</p>
                )}
              </div>
...
export default CreateOrEditPixele;