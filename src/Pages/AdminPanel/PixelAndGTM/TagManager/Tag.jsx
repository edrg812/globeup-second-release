...
const RenderTagDetailsRow = (item, index) => {
  
  return (
    <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
      <td className="py-3 px-2 align-middle">{index + 1}</td>
      <td className="py-3 px-2 align-middle">{item.tagsId}</td>
...
const TagSetting = () => {
  ...
  const tagData = [
    { id: 1, tagsId: '1296348748317817',  status: 'Active' },
    ...
    <div className="bg-gray-50 font-sans" style={{ backgroundColor: '#f3e8ff' }}>
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-800">tag Manage</h1>
          <Link 
          to={`/admin/tag/create`}
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <Table
            columns={columns}
            data={tagData}
            renderRow={RenderTagDetailsRow}
          />

          <div className="flex flex-col md:flex-row justify-between items-center mt-4 space-y-2 md:space-y-0">
            <span className="text-gray-600 text-sm">Showing 1 to {tagData.length} of {tagData.length} entries</span>
