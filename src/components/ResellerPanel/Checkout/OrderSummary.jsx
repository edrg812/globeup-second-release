...
const OrderSummary = ({
  ...
  incrementQuantity,
  decrementQuantity,
  resellingPrices, // from parent
  onResellingPriceChange, // from parent
}) => {
  ...
  const totalReselling = cart.reduce(
    (total, item) =>
      total +
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm table-auto">
          <thead className="bg-yellow-50 text-gray-700">
            <tr>
              ...
              <th className="p-2">Reselling Price</th>
          ...
          <tbody>
            {cart.map((item, index) => {
              ...
              const itemTotal = resellerPrice * (item.quantity || 1);
                ...
                <tr key={index} className="border-t">
                  ...
                  <td className="text-center p-2">
                    <input
                      ...
                      min={item.price}
                      value={
                        // If the user has cleared the field, show empty string
                        resellingPrices[item.product_variant_id] === ""
                          ? ""
                          : resellingPrices[item.product_variant_id]
                      }
                      onChange={(e) => {
        ...
        <div className="text-right mt-4 text-sm text-gray-600">
          <p>
            ...
            (Wholesale): ৳ {subtotal.toLocaleString()}
          </p>
          <p>Reselling Total: ৳ {totalReselling.toLocaleString()}</p>
        </div>
      </div>
    </div>
  );
